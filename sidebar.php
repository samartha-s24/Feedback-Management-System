...
function _nav_item(string $href, string $icon, string $label, string $current, string $badge = ''): string {
    ...
    $badgeHtml = $badge
        ...
        : '';
    return "<a href=\"{$href}\" class=\"{$cls}\" onclick=\"if (window.innerWidth < 768) { closeFacultySidebar(); }\">
...
?>
<!-- Mobile sidebar overlay (hidden on md+, toggled by JS below) -->
<div id="sidebar-overlay"
     class="fixed inset-0 bg-black/50 z-10 opacity-0 pointer-events-none transition-opacity duration-300 md:hidden"
     onclick="closeFacultySidebar()"></div>
...
<aside id="faculty-sidebar"
    ...
    <div class="h-16 flex items-center px-4 border-b border-gray-200 dark:border-slate-700 flex-shrink-0">
        ...
        <button type="button" onclick="closeFacultySidebar()"
                class="ml-auto md:hidden text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 rounded-lg">
...
<script>
    // Faculty sidebar mobile drawer controls (used by the hamburger button in header.php)
    ...
    // Close on Escape, and auto-reset state if resized to desktop
    document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape') closeFacultySidebar();
    });
    window.addEventListener('resize', function () {
        if (window.innerWidth >= 768) closeFacultySidebar();
    });
</script>
